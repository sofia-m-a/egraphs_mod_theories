...
The distinction between equations and identities is important. Equations are easy, in the sense that the congruence-closure algorithm \ref{egraphs} is already enough to solve the problem entirely when we have only equations (and so $I = \{\}$).
...
Equality saturation would terminate with the E-graph \ref{fig:eqsat1}, as for every match in the E-graph of either side of $\forall x.\; f(x) = x$, the other side is in the E-graph and in the same E-class.
...
\fxerror{Diagrams need redoing. Wordiness fixed by commenting out things, for now.... I'm still unsure if this is the right example to be using for this section}
...
\chapter{Which theories do E-graphs struggle with?} \label{howfare}
